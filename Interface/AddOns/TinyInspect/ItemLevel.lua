...
-- function UpdateSlotByTiny(self, bagID, slotID)
	-- local slot = self.Bags[bagID][slotID];
	-- local clink = GetContainerItemLink(bagID, slotID);
	-- if TinyInspectDB['EnableItemLevelBag'] and slot.itemLevel then slot.itemLevel:SetText("") end
	-- SetItemLevel(slot, clink, "Bag", bagID, slotID)	
-- end
...
-- LibEvent:attachTrigger("ITEMLEVEL_FRAME_SHOWN", function(self, frame, parent, category)
    ...
    -- if (frame.anchorPoint ~= anchorPoint) then
		...
		-- local x, y  = 0, 0  --by eui.cc
		-- if anchorPoint:find('TOP') and category == 'PaperDoll' then y = -2 else y = 2 end
		-- if anchorPoint:find('LEFT') and category == 'PaperDoll' then x = 2 else x = -2 end
        -- frame.levelString:SetPoint(anchorPoint or "TOP", x, y)
    -- end
-- end)
